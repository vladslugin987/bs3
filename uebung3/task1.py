...
shared_array = None
next_position = None
array_lock = None  # Lock for protecting the critical section

def process_1():
    """Process that writes '1' to the array 10 times."""
    global shared_array, next_position, array_lock
    
    for _ in xrange(10):  # xrange instead of range in Python 2
        # Acquire the lock before entering the critical section
        array_lock.acquire()
        try:
            # Critical section - Get position, write value, increment position
        ...
        # Outside the critical section, can add small sleep
...
def process_2(delay_iterations=1000000):
    ...
    for _ in xrange(10):  # xrange instead of range in Python 2
        ...
        array_lock.acquire()
        try:
            # Critical section - Get position
            pos = next_position.value
            
            # Delay loop moved inside the critical section to demonstrate lock effect
            for _ in xrange(delay_iterations):  # xrange instead of range
                pass
...
def run_experiment(start_order, delay_iterations):
    ...
    array_lock = Lock()  # Create the lock
    
    # Initialize array with zeros
    for i in xrange(len(shared_array)):  # xrange instead of range
        shared_array[i] = 0
...
if __name__ == "__main__":
    ...
    for start_order, delay in test_configs:
        ...
        for run in xrange(3):  # xrange instead of range
